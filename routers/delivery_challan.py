...
async def update_buyer(request: Request, id: int, input_buyer: BuyerIn):
    ...
    request.app.state.cache.challans = [ChallanCache(**challan.dict(exclude={"buyer"}), buyer=challan.buyer if challan.buyer.id != id else edited_buyer) for challan in request.app.state.cache.challans]
    return edited_buyer
